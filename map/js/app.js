...
flatData.forEach(row => {
    ...
    if (!map.has(key)) {
        ...
        map.set(key, {
            ...
            regZone:   reg.zone,     // 'A' | 'B' | null
            regLabels: reg.labels,   // 배지 문자열 배열
        });
    }
    const g = map.get(key);
    g.rows.push(row);
...
if (g.regLabels && g.regLabels.length > 0) {
    regBadgesHTML = g.regLabels.map(label => {
        let cls = 'reg-badge';
        if (label === '투기지역')      cls += ' reg-A';
        else if (label === '투기과열지구') cls += ' reg-B';
        else if (label === '조정대상지역') cls += ' reg-C';
