...
function _checkShareLimit() {
  const limit = (_C && _C.REPORT_COPY_DAILY_LIMIT) ? _C.REPORT_COPY_DAILY_LIMIT : 10;
...
async function _shortenUrl(longUrl) {
  try {
    const api = (_C && _C.SHORTENER_API) ? _C.SHORTENER_API : 'https://is.gd/create.php?format=simple&url=';
...
function _forceCopy(text, successMsg) {
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text)
      .then(function() { if (successMsg) showAlert(successMsg, null, '✅'); })
      .catch(function() { _fcFallback(text, successMsg); });
  } else {
...
function _fcFallback(text, successMsg) {
  ...
  try {
    ...
    showAlert('복사에 실패했습니다.<br>직접 복사해 주세요.<br><br>' + text, null, '⚠️');
  }
